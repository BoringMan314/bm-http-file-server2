...
Enter the destination folder.=移動先フォルダーを入力してください。
...
Failed=失敗
files=ファイル
files uploaded correctly.= 件のファイルが正常にアップロードされました
files were moved.= 件のファイルが移動されました。
Folder=フォルダー
folders=フォルダー
forbidden=禁止
Forbidden=アクセス禁止
...
Logout=ログアウト
Mask=マスク
max s dl msg=このサーバーには<b>同時</b>ダウンロード数の制限があります。<br>制限に達しました。後でもう一度試してください。
Messages=メッセージ
...
Password changed, you'll have to login again.=パスワードが変更されました。再度ログインしてください。
...
Where to search=検索範囲
You are banned=あなたは禁止されています
You are banned.=あなたは禁止されています
You must select a single item to rename=名前を変更するには、1 つの項目だけを選択してください
You must select a single item to rename.=名前を変更するには、1 つの項目だけを選択してください
