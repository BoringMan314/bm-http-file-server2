...
Either your user name and password do not match, or you are not permitted to access this resource.=您所輸入的帳號密碼錯誤，或您沒有權限訪問所在的資源。
...
Failed=失敗
files=檔案
files uploaded correctly.=個檔案已被上傳。
files were moved.=個檔案已移動。
Folder=檔案目錄
folders=資料夾
forbidden=禁止
Forbidden=禁止存取
...
Logout=登出
Mask=隱藏
max s dl msg=此伺服器同時下載數量有限制。<br>目前已達上限，請稍後再試。
Messages=訊息
More options=更多選項
Move=移動檔案
Move.=移動檔案
Moving items to=正在移動項目到
Name=名稱
NEW=新
New folder=建立新資料夾
New folder.=建立新資料夾
...
Password changed, you'll have to login again.=密碼已變更，你需要重新登入。
...
Server time=伺服器時間
Server time.=伺服器時間
Server Too Busy=伺服器忙碌中
Server Too Busy.=伺服器忙碌中
Server uptime=伺服器運作時間
Server uptime.=伺服器運作時間
Size=大小
...
Sort by=排序依據
Speed=速度
The server is too busy to handle your request at this time. Retry later=目前伺服器忙碌中，無法回應您的請求，請稍後再試。
The server is too busy to handle your request at this time. Retry later.=目前伺服器忙碌中，無法回應您的請求，請稍後再試。
this folder and sub-folders=此資料夾與子資料夾
this folder only=僅此資料夾
This resource is not accessible.=此資源禁止訪問。
Time left=剩餘時間
Time left.=剩餘時間
Timestamp=修改時間
title=標題
...
Where to search=搜尋範圍
You are banned=很抱歉，您已被禁封。
You are banned.=很抱歉，您已被禁封。
You must select a single item to rename=請選擇一個檔案以重新命名
You must select a single item to rename.=請選擇一個檔案以重新命名。
